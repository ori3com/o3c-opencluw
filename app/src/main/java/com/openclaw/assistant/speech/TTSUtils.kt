...
object TTSUtils {
    ...
    fun splitTextForTTS(text: String, maxLength: Int = 1000): List<String> {
        ...
        while (offset < text.length) {
            ...
            if (remainingLength <= maxLength) {
                val chunk = text.substring(offset).trim()
            ...
            val nextOffset = if (splitIndex > offset) splitIndex else limit
            val chunk = text.substring(offset, nextOffset).trim()
            if (chunk.isNotBlank()) chunks.add(chunk)
            offset = nextOffset
        }
    ...
    private fun findBestSplitPoint(text: String, offset: Int, limit: Int): Int {
        ...
        val minAcceptablePos = offset + searchLength / 3

        // Priority: paragraph break > sentence end > comma > space
        val paragraphBreak = findLastIndexOf(text, "\n\n", offset, limit)
        if (paragraphBreak > offset + searchLength / 2) return paragraphBreak + 2

        var bestPos = -1
        for (ender in SENTENCE_ENDERS) {
            val pos = findLastIndexOf(text, ender, offset, limit)
            if (pos > bestPos) bestPos = pos + ender.length
        }
        if (bestPos > minAcceptablePos) return bestPos

        val lineBreak = findLastIndexOf(text, "\n", offset, limit)
        if (lineBreak > minAcceptablePos) return lineBreak + 1
        ...
        val space = findLastIndexOf(text, " ", offset, limit)
        if (space > minAcceptablePos) return space + 1

        return -1
    }

    private fun findLastIndexOf(text: String, delimiter: String, offset: Int, limit: Int): Int {
        if (limit - offset < delimiter.length) return -1
